...
start_my_httpd(){
    ...
    chcon -R -t httpd_user_ra_content_t  /opt/DATA/httpdFiles  

    # 设置目录权限(增加所有人的可读权限)
    chmod -R o+r  /opt/DATA/httpdFiles

    # 检查返回值
    if [ $? -eq 0 ]; then
        log_message "chmod命令执行成功，/opt/DATA/httpdFiles 目录下文件权限已更改为所有人可读。"
    else
        log_message "chmod命令执行失败，/opt/DATA/httpdFiles 目录下文件权限修改过程遇到问题。状态码：$?"
        sudo_execute "chmod -R o+r  /opt/DATA/httpdFiles" 
    fi
...
main (){
    ...
    # 使用getopt解析参数
    local ltmp_TEMP=$(getopt -o hvtpxf: --long help,version,start,stop,test:,project:,fs:,port:,dev:,debug,trace,init_new_disk:,permanent,zone:,type:,add_fw_port:,add_NTP_Server:,remove_fw_port:,nfs_share:,icmp_reply:,set_x11vnc:,py_install:,url:,file: -n "$0" -- "$@")
    eval set -- "$ltmp_TEMP"
